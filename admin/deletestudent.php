<?php
        include("db/dbase.php");
         $id=$_GET['eid'];
          ...
          $lev=$row['seslevid'];
          if($lev=='1'){
          $sql = $con->prepare("DELETE  FROM studentsesslevel WHERE sid='$id' and seslevid='1'");
          $sql->execute();
          $sql = $con->prepare("DELETE  FROM student WHERE sid='$id'");
          $sql->execute(); 
          $sql = $con->prepare("DELETE  FROM studentannoucement WHERE sid='$id'");
          $sql->execute(); 
          $sql = $con->prepare("DELETE  FROM marks WHERE sid='$id' and level='level 1'");
          $sql->execute();
          $sql = $con->prepare("DELETE  FROM claim WHERE sid='$id' and level='level 1'");
          $sql->execute();
          }else if($lev=='2'){
